...
export const RULES = {
  ...
  // The Vakta picker holds an object `{ name, userId }` (in-house) or just a
  // typed name (external). Its value is present only when a NAME is set, so
  // required validates the name, not the wrapper object.
  vaktaRequired: (v) => {
    const name = v && typeof v === 'object' ? v.name : v;
    return isBlank(name) ? 'This field is required.' : null;
  },
