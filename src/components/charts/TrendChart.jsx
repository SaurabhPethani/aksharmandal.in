...
/** The viewBox width. The SVG scales to whatever the card gives it. */
const VB_W = 720;
...
export default function TrendChart({
  ...
  const pick = event => {
    if (!width) return;
    const vbX = (event.nativeEvent.locationX / width) * VB_W;
    const at =
      points.length === 1
        ? 0
  ...
  const tooltipLeft = (() => {
    ...
    const at = (x(picked) / VB_W) * width;
    const flip = picked > points.length / 2;
    return Math.max(
      4,
      Math.min(width - TOOLTIP_W - 4, flip ? at - TOOLTIP_W - 8 : at + 8),
    );
  })();
  ...
  return (
    <View
      ...
      // Only a sideways drag scrubs the chart; an up/down swipe still scrolls
      ...
      >
        ...
        {pointLabel &&
          points.map((p, i) => {
            ...
            const lines = (Array.isArray(result) ? result : [result]).filter(
              l => l != null && l !== '',
            );
